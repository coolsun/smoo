...
<template>
  <form class="needs-validation" novalidate @submit.prevent="updateProperties">
    <mdbRow v-show="editMode||dataArray.length >= 1">
      ...
      <mdbCol col="12" md="2">
        <label>{{ $t('device_types.name') }}</label>
      </mdbCol>

      <mdbCol col="2">
        <label>{{ $t('device_types.kind') }}</label>
      </mdbCol>

      <mdbCol col="2">
        <label>{{ $t('device_types.unit') }}</label>
      </mdbCol>

      <mdbCol col="2">
        <label>{{ $t('device_types.required') }}</label>
      </mdbCol>
    </mdbRow>
    ...
    <mdbRow v-for="(property, index) in dataArray" :key="index" v-if="property._destroy != '1'">
      ...
      <mdbCol class="form-group" col="2">
        <select class="form-control" v-model="property.kind" required :disabled="!editMode">
          <option value="" selected hidden>{{ $t('device_types.choose_one') }}</option>
          <option v-for="kind in kindsOptions" :value="kind">{{ kind }}</option>
        </select>
      </mdbCol>
      ...
      <mdbCol class="form-group" col="2">
        <input class="form-control" type="text" v-model="property.required" required :disabled="!editMode">
      </mdbCol>
...
export default {
  name: 'DeviceNormalizationProperties',
  components: {
    mdbRow,
    mdbCol,
    mdbBtn,
  ...
  methods: {
    getPropertiesInfo() {
      ...
      this.$axios.get(`/apis/device_types/${this.editID}/get_normalization_properties`)
        .then(response => {
          this.dataArray = response.data.device_type.normalization_properties;
          this.kindsOptions = response.data.kinds;
          this.unitOptions = response.data.units;
        })
        .catch(error => {
    ...
    addProperty() {
      this.dataArray.push({name: '', kind: '', unit: '', required: true});
    },
    ...
    updateProperties(event) {
      if (event.target.checkValidity()) {
        ...
        var updateData = {
          device_type: {
            normalization_properties_attributes: this.dataArray.concat(this.removeArray)
          }
        };
        this.$axios.put(`/apis/device_types/${this.editID}/update_normalization_properties`, updateData)
          .then(() => {
            location.reload();
          })
          .catch(error => {
