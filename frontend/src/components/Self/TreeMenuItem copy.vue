<template>
  <div class="tree-menu-item" :class="className">
    <div class="p-1" :class="headerClass" @click="opened = !opened" style="position: relative">
      <a v-if="nodes" class="p-0 m-0 z-depth-0 stretched-link" @click="show = !show">
    ...
    <transition v-if="(animated || colorful) && nodes" name="slide" >
    ...
    <ul v-else-if="show && nodes" class="nested list-unstyled pl-5">
      <slot></slot>
      <app-tree-menu-item
      v-if="show"
      v-for="node in nodes" 
